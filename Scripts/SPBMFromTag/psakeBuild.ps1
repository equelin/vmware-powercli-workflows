Include -fileNamePathToInclude "$PSScriptRoot\Functions\Get-StoragePolicyFromTag.ps1"
Include -fileNamePathToInclude "$PSScriptRoot\Functions\Update-StoragePolicyFromTag.ps1"
...
task Remediate -depends Connection, Analyze, Test {
    Try {
        Get-Datacenter $cfg.scope.datacenter | Get-Cluster $cfg.Scope.cluster | Get-VM $cfg.scope.vm | Get-StoragePolicyFromTag -TagCategory $cfg.TagCategory | Update-StoragePolicyFromTag -Confirm:$false
    }
    Catch {
