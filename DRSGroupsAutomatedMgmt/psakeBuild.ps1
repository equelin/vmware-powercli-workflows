Include -fileNamePathToInclude "$PSScriptRoot\Functions\Update-DRSVMGroup.ps1"
...
task Analyze -depends Connection {
    $saResults = Invoke-ScriptAnalyzer -Path .\Functions\Update-DRSVMGroup.ps1 -ExcludeRule PSAvoidGlobalVars,PSAvoidUsingWriteHost -Severity @('Error', 'Warning') -Recurse -Verbose:$false
    if ($saResults) {
...
task Remediate -depends Connection, Analyze, Test {
    ...
    Try {
        Get-Datacenter $cfg.scope.datacenter | Get-Cluster $cfg.Scope.cluster | Get-VM $cfg.scope.vm | Update-DRSVMGroup -Cfg $cfg -Server $Server -Confirm:$false
    }
    Catch {
